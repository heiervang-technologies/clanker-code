...
impl ChatWidget {
    ...
    fn collaboration_mode_indicator(&self) -> Option<CollaborationModeIndicator> {
        ...
        match self.active_mode_kind() {
            ModeKind::Larp => Some(CollaborationModeIndicator::Larp),
            ModeKind::Plan => Some(CollaborationModeIndicator::Plan),
            ModeKind::LockedIn => Some(CollaborationModeIndicator::LockedIn),
            ModeKind::Based => Some(CollaborationModeIndicator::Based),
            ModeKind::Cringe => Some(CollaborationModeIndicator::Cringe),
            ModeKind::Ultrachill => Some(CollaborationModeIndicator::Ultrachill),
    ...
    pub(super) fn update_collaboration_mode_indicator(&mut self) {
        ...
        let larp = mode_indicator == Some(CollaborationModeIndicator::Larp);
        let goal_indicator = if mode_indicator.is_none() || larp {
            self.goal_status_indicator(Instant::now())
        } else {
            None
        };
        let indicator = if larp && goal_indicator.is_some() {
            None
        } else {
            mode_indicator
        };
    ...
    pub(super) fn refresh_goal_status_indicator_for_time_tick(&mut self) {
        let mode_indicator = self.collaboration_mode_indicator();
        if mode_indicator.is_some_and(|indicator| indicator != CollaborationModeIndicator::Larp) {
            return;
        }
    ...
    /// Cycle to the next collaboration mode variant in picker order.
