...
impl CollaborationModeIndicator {
    fn label(self, show_cycle_hint: bool) -> String {
        ...
        match self {
            CollaborationModeIndicator::Larp => format!("LARP mode{suffix}"),
            CollaborationModeIndicator::Plan => format!("Plan mode{suffix}"),
            CollaborationModeIndicator::LockedIn => format!("Locked In mode{suffix}"),
            CollaborationModeIndicator::Based => format!("Based mode{suffix}"),
            CollaborationModeIndicator::Cringe => format!("Cringe mode{suffix}"),
            CollaborationModeIndicator::Ultrachill => format!("Ultrachill mode{suffix}"),
    ...
    fn styled_span(self, show_cycle_hint: bool) -> Span<'static> {
        ...
        // Founder-specified palette (Markus, 2026-07-17): LARP purple,
        // Locked In red, Based blue, Cringe orange, Ultrachill cyan, Plan
        // white. This indicator is exempt from the general color guidance in
        // styles.md; see the "Collaboration mode indicator" note there.
        match self {
            CollaborationModeIndicator::Larp => Span::from(label).magenta(),
            #[allow(clippy::disallowed_methods)] // white is part of the specified palette
            CollaborationModeIndicator::Plan => Span::from(label).white(),
            CollaborationModeIndicator::LockedIn => Span::from(label).red(),
            CollaborationModeIndicator::Based => Span::from(label).blue(),
            #[allow(clippy::disallowed_methods)] // orange has no ANSI slot
            CollaborationModeIndicator::Cringe => Span::from(label).fg(Color::Rgb(255, 140, 0)),
            CollaborationModeIndicator::Ultrachill => Span::from(label).cyan(),
...
mod tests {
    ...
    fn footer_snapshots() {
        ...
        snapshot_footer_with_mode_indicator(
            "footer_mode_indicator_larp",
            /*width*/ 120,
            &props,
            Some(CollaborationModeIndicator::Larp),
        );

        snapshot_footer_with_mode_indicator(
            "footer_mode_indicator_locked_in",
            /*width*/ 120,
            &props,
            Some(CollaborationModeIndicator::LockedIn),
        );

        snapshot_footer_with_mode_indicator(
            "footer_mode_indicator_based",
            /*width*/ 120,
            &props,
            Some(CollaborationModeIndicator::Based),
        );

        snapshot_footer_with_mode_indicator(
            "footer_mode_indicator_cringe",
            /*width*/ 120,
            &props,
            Some(CollaborationModeIndicator::Cringe),
        );

        snapshot_footer_with_mode_indicator(
            "footer_mode_indicator_ultrachill",
            /*width*/ 120,
            &props,
            Some(CollaborationModeIndicator::Ultrachill),
        );
