...
fn run_resolve(
    ...
    let mut command = Command::new(codex_utils_cargo_bin::cargo_bin("clanker")?);
    command
...
fn json_output(assertion: &assert_cmd::assert::Assert) -> anyhow::Result<Value> {
    Ok(serde_json::from_slice(&assertion.get_output().stdout)?)
}

fn entry_names(path: &std::path::Path) -> std::io::Result<Vec<String>> {
...
#[test]
fn resolve_without_materialization_is_read_only() -> anyhow::Result<()> {
...
#[test]
fn materialize_builtin_resolves_chloe_on_first_use() -> anyhow::Result<()> {
    let home = TempDir::new()?;
...
#[test]
fn materialize_rusty_alias_creates_only_clanker() -> anyhow::Result<()> {
    let home = TempDir::new()?;
...
#[test]
fn materialize_custom_name_does_not_create_builtins() -> anyhow::Result<()> {
    let home = TempDir::new()?;
...
#[test]
fn materialize_ignores_unrelated_partial_builtin_directory() -> anyhow::Result<()> {
    let home = TempDir::new()?;
    let partial = home.path().join("characters/clanker");
    fs::create_dir_all(&partial)?;
    fs::write(partial.join("sentinel"), b"untouched")?;
    ...
    assert_eq!(fs::read(partial.join("sentinel"))?, b"untouched");
    Ok(())
}

#[test]
fn materialize_selected_partial_builtin_returns_json_failure() -> anyhow::Result<()> {
    let home = TempDir::new()?;
