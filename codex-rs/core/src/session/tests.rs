    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            codex_protocol::protocol::TurnStartedEvent {
                ...
                collaboration_mode_kind: ModeKind::Larp,
            },
        )),
...
#[tokio::test]
async fn collaboration_mode_switch_sends_full_doc_once_then_one_line_reminder() {
    let (session, mut turn_context) = make_session_and_context().await;
    let previous_context_item = turn_context.to_turn_context_item();

    let full_doc = "Full Locked In mode document with detailed posture guidance.";
    turn_context.mode = ModeKind::LockedIn;
    turn_context.collaboration_mode_developer_instructions = Some(full_doc.to_string());

    let first_switch_items = session
        .build_settings_update_items(Some(&previous_context_item), &turn_context)
        .await;
    let first_switch_text = collect_message_texts(&first_switch_items);
    assert!(
        first_switch_text.contains(full_doc),
        "first switch should send the full mode document, got: {first_switch_text}"
    );
    ...
    let second_switch_items = session
        .build_settings_update_items(Some(&previous_context_item), &turn_context)
        .await;
    let second_switch_text = collect_message_texts(&second_switch_items);
    assert!(
        !second_switch_text.contains(full_doc),
        "repeat switch should not resend the full mode document, got: {second_switch_text}"
    );
    assert!(
        second_switch_text
            .contains("Your earlier Locked In mode instructions in this conversation apply"),
        "repeat switch should send the one-line reminder, got: {second_switch_text}"
    );
}

fn collect_message_texts(items: &[ResponseItem]) -> String {
    items
        .iter()
        .filter_map(|item| match item {
