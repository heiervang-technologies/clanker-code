...
impl Session {
    ...
    async fn build_settings_update_items(
        ...
        let collaboration_mode_full_render =
        ...
        let (previous_turn_settings, collaboration_mode_full_in_history) = {
            let state = self.state.lock().await;
            let full_in_history = collaboration_mode_full_render.as_deref().is_some_and(|full| {
                state.history.raw_items().iter().any(|item| {
                    matches!(item, ResponseItem::Message { content, .. } if content.iter().any(
                        |content_item| matches!(content_item, ContentItem::InputText { text } if text.contains(full)),
                    ))
                })
            });
            (state.previous_turn_settings(), full_in_history)
        };
        ...
        crate::context_manager::updates::build_settings_update_items(
            ...
            collaboration_mode_full_in_history,
        )
    }
