...
fn build_collaboration_mode_update_item(
    previous: Option<&TurnContextItem>,
    next: &TurnContext,
    full_instructions_in_history: bool,
) -> Option<String> {
    ...
    if prev.collaboration_mode.as_ref() != Some(&collaboration_mode) {
        ...
        //
        // The full mode document is sent at most once per thread; if it is
        // already in the prompt history, a one-line switch notice is enough
        // and avoids re-paying the full document on every mode switch.
        let instructions = if full_instructions_in_history {
            CollaborationModeInstructions::switch_reminder(&collaboration_mode)?
        } else {
            CollaborationModeInstructions::from_collaboration_mode(&collaboration_mode)?
        };
        Some(instructions.render())
    } else {
        None
    }
}
...
pub(crate) fn build_settings_update_items(
    ...
    exec_policy: &Policy,
    personality_feature_enabled: bool,
    collaboration_mode_full_in_history: bool,
) -> Vec<ResponseItem> {
    ...
    let developer_update_sections = [
        ...
        build_collaboration_mode_update_item(previous, next, collaboration_mode_full_in_history),
