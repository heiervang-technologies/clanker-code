...
fn request_user_input_unavailable_messages_respect_default_mode_feature_flag() {
    ...
    assert_eq!(
        request_user_input_unavailable_message(ModeKind::Larp, &default_available_modes()),
        Some("request_user_input is unavailable in LARP mode".to_string())
    );
    assert_eq!(
...
fn request_user_input_tool_description_mentions_available_modes() {
    ...
    assert_eq!(
        ...
        "Request user input for one to three short questions and wait for the response. Set autoResolutionMs, from 60000 to 240000 milliseconds, only when the question is useful but non-blocking and continuing with best judgment is acceptable if the user does not answer; omit it when explicit user input is required. This tool is only available in LARP or Plan mode.".to_string()
    );
}
