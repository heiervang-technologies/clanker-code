...
async fn request_user_input_rejected_in_default_mode_by_default() -> anyhow::Result<()> {
    assert_request_user_input_rejected("LARP", |model| CollaborationMode {
        mode: ModeKind::Larp,
        settings: Settings {
            model,
            reasoning_effort: None,
...
async fn request_user_input_round_trip_in_default_mode_with_feature() -> anyhow::Result<()> {
    request_user_input_round_trip_for_mode(ModeKind::Larp, /*auto_resolution_ms*/ None).await
}
