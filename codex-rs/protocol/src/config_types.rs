...
pub enum ModeKind {
    Plan,
    // The default collaboration mode. Legacy wire values deserialize to it.
    #[default]
    #[serde(
        alias = "default",
        alias = "code",
    ...
    Larp,
    // Ultra-focused execution: concise, persistent, task-first.
    LockedIn,
    // Assertive and decisive; posture only, authority boundaries unchanged.
    Based,
    // Cautious and low-assumption; checks before consequential moves.
    Cringe,
    // Maximum low-pressure posture: relaxed, patient, unhurried.
    Ultrachill,
    #[doc(hidden)]
...
pub const TUI_VISIBLE_COLLABORATION_MODES: [ModeKind; 6] = [
    ModeKind::Larp,
    ModeKind::LockedIn,
    ModeKind::Based,
    ModeKind::Cringe,
...
/// User-configured overrides for a built-in collaboration mode preset.
///
/// Set via `[collaboration_modes.<mode>]` in config.toml to customize what a
/// mode implies without forking the built-in preset list. Unset fields keep
/// the built-in behavior.
#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize, JsonSchema, TS)]
pub struct CollaborationModeOverride {
    /// Replaces the mode's built-in developer-instruction template.
    pub developer_instructions: Option<String>,
    /// Overrides the reasoning effort the mode selects.
    pub reasoning_effort: Option<ReasoningEffort>,
    /// Pins the mode to a specific model.
    pub model: Option<String>,
}

/// Apply user-configured per-mode overrides on top of built-in presets.
///
/// Presets keep their list order; overrides for modes not present in
/// `presets` are ignored.
pub fn apply_collaboration_mode_overrides(
    presets: Vec<CollaborationModeMask>,
    overrides: &HashMap<ModeKind, CollaborationModeOverride>,
) -> Vec<CollaborationModeMask> {
    presets
        .into_iter()
        .map(|mut mask| {
            if let Some(mode_override) = mask.mode.and_then(|mode| overrides.get(&mode)) {
                if let Some(instructions) = &mode_override.developer_instructions {
                    mask.developer_instructions = Some(Some(instructions.clone()));
                }
                if let Some(effort) = mode_override.reasoning_effort.clone() {
                    mask.reasoning_effort = Some(Some(effort));
                }
                if let Some(model) = &mode_override.model {
...
mod tests {
    ...
    #[test]
    fn mode_kind_deserializes_alias_values_to_larp() {
        for alias in ["default", "code", "pair_programming", "execute", "custom"] {
            ...
            assert_eq!(ModeKind::Larp, mode);
        }
    }

    #[test]
    fn mode_kind_round_trips_visible_wire_names() {
        let cases = [
            (ModeKind::Larp, "\"larp\""),
        ...
        for (mode, wire) in cases {
            assert_eq!(wire, serde_json::to_string(&mode).expect("serialize mode"));
            let parsed: ModeKind = serde_json::from_str(wire).expect("deserialize mode");
            assert_eq!(mode, parsed);
        }
    }

    #[test]
    fn collaboration_mode_overrides_preserve_order_and_builtin_fields() {
        let presets = vec![
            CollaborationModeMask {
                name: "LARP".to_string(),
                mode: Some(ModeKind::Larp),
                model: None,
                reasoning_effort: None,
                developer_instructions: Some(Some("built-in LARP".to_string())),
            },
            CollaborationModeMask {
                name: "Plan".to_string(),
                ...
                developer_instructions: Some(Some("built-in Plan".to_string())),
            },
        ];
        let overrides = HashMap::from([
            (
                ...
                CollaborationModeOverride {
                    developer_instructions: Some("custom LARP".to_string()),
                    reasoning_effort: Some(ReasoningEffort::High),
            ...
            (
                ModeKind::Based,
                CollaborationModeOverride {
                    developer_instructions: Some("unused".to_string()),
                    reasoning_effort: None,
                    model: None,
                },
            ),
        ...
        assert_eq!(
            apply_collaboration_mode_overrides(presets, &overrides),
            vec![
                CollaborationModeMask {
                    name: "LARP".to_string(),
                    mode: Some(ModeKind::Larp),
                    ...
                    developer_instructions: Some(Some("custom LARP".to_string())),
                },
                CollaborationModeMask {
                    name: "Plan".to_string(),
                    mode: Some(ModeKind::Plan),
